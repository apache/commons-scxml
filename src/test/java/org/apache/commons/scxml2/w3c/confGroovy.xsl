...
<xsl:stylesheet
	...
	<xsl:template match="//@conf:datamodel">
		<xsl:attribute name="datamodel">groovy</xsl:attribute>
	</xsl:template>
  ...
  is the second argument -->
	...
	<xsl:template match="//@conf:eventdataVal">
		<xsl:variable name="op"><xsl:value-of select="."/></xsl:variable>
		<xsl:choose>
			<xsl:when test="starts-with($op,'''')">
	...
	<xsl:template match="//@conf:varPrefix">
		<xsl:attribute name="cond">
			<xsl:analyze-string select="."
													regex="(\w+)(\W)(\w+)">
				<xsl:matching-substring>
					<!-- input "2 1" generates: "!empty(Var1) and !empty(Var2) and Var1.startsWith(Var2)" -->
					<xsl:text>!empty(Var</xsl:text><xsl:value-of select="regex-group(3)"/><xsl:text>) and !empty(Var</xsl:text><xsl:value-of select="regex-group(1)"/><xsl:text>) and Var</xsl:text><xsl:value-of select="regex-group(3)"/><xsl:text>.startsWith(Var</xsl:text><xsl:value-of select="regex-group(1)"/><xsl:text>)</xsl:text>
				</xsl:matching-substring>
			</xsl:analyze-string>
		</xsl:attribute>
	</xsl:template>
	...
	<xsl:template match="//@conf:isBound">
		<xsl:attribute name="cond">var('Var<xsl:value-of select="." />')</xsl:attribute>
	</xsl:template>
	...
	<xsl:template match="//@conf:eventLanguageMatchesDatamodel">
		<xsl:attribute name="cond"> _event.language == 'groovy'</xsl:attribute>
	</xsl:template>
